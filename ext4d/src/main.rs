...
use fs::BlockDev;
use ulib::{open, read, recv_ipc_blocking, send_ipc, write, IpcMsg, IPC_MAX_DATA};
...
fn handle(state: &mut State, req: &IpcMsg, reply: &mut IpcMsg) {
	...
	match req.id {
		proto::MSG_LOOKUP => {
			...
			match dir::lookup_in_dir(state.dev, &state.sb, &state.bgdt, &parent, name) {
				Some(child_ino) => {
					...
					reply.data[4] = child
						.map(|i| if i.is_dir() { 2u8 } else { 1u8 })
						.unwrap_or(0);
				}
				None => {
		...
		proto::MSG_MKDIR => {
			...
			let name = core::str::from_utf8(&d[5..5 + nlen_clamped]).unwrap_or("");
			let child_ino =
				match bitmap_alloc::alloc_inode(state.dev, &mut state.sb, &mut state.bgdt) {
					Some(i) => i,
					None => {
						reply.data[0..4].copy_from_slice(&0u32.to_le_bytes());
						return;
			...
			use fs::ext4::inode::{EXT4_EXTENTS_FL, EXT4_S_IFDIR};
			let mut child = Inode {
		...
		proto::MSG_CREATE => {
			...
			use fs::ext4::inode::{EXT4_EXTENTS_FL, EXT4_S_IFREG};
			let child = Inode {
