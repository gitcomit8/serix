...
impl FsDriver for Ext4Driver {
	...
	fn probe(&self, dev: &dyn BlockDev) -> bool {
		...
		use superblock::{INCOMPAT_64BIT, INCOMPAT_EXTENTS};
		sb.feature_incompat & INCOMPAT_EXTENTS != 0 && sb.feature_incompat & INCOMPAT_64BIT == 0
	}
	...
	fn mount(&self, _dev: Arc<dyn BlockDev>) -> Option<Arc<dyn INode>> {
		 ...
		 */
		if ::ipc::IPC_GLOBAL
			.get_port(kernel_stub::EXT4_REQ_PORT_VAL)
			.is_none()
		{
