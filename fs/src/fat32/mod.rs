...
fn fat_alloc_cluster(bpb: &Bpb) -> Option<u32> {
	let max_cluster =
		(bpb.total_sectors - bpb.data_start_sector() as u32) / bpb.sectors_per_cluster as u32 + 2;
	...
	for cluster in chain {
		...
		for s in 0..bpb.sectors_per_cluster as u64 {
			...
			for entry_idx in 0..(SECTOR_SIZE / DIR_ENTRY_SIZE) {
				...
				if raw[11] & ATTR_VOLUME_ID != 0 && raw[11] & ATTR_DIRECTORY == 0 {
					lfn_buf.clear();
					lfn_start = None;
					continue;
				}
				...
				entries.push(DirEntry {
					name,
					attr,
					first_cluster: first_cluster_val,
					size,
					entry_sector: sector,
					entry_offset: offset,
					lfn_start_sector: ls,
...
fn find_entry_in_dir(bpb: &Bpb, dir_cluster: u32, name: &str) -> Option<DirEntry> {
	read_dir_entries(bpb, dir_cluster)
		.into_iter()
...
impl INode for FatDirINode {
	...
	fn insert(&self, name: &str, _node: Arc<dyn INode>) -> Result<(), &'static str> {
		...
		create_dir_entry(bpb, self.cluster, name, ATTR_ARCHIVE, new_cluster)
	...
	fn create_file(&self, name: &str) -> Result<Arc<dyn INode>, &'static str> {
		...
		let new_cluster = fat_alloc_cluster(bpb).ok_or("disk full")?;
		let (es, eo) = create_dir_entry(bpb, self.cluster, name, ATTR_ARCHIVE, new_cluster)
			.ok_or("dir entry creation failed")?;
	...
	fn mkdir(&self, name: &str) -> Result<(), &'static str> {
		...
		dotdot[16] = d[0];
		dotdot[17] = d[1];
		dotdot[20] = ((self.cluster >> 16) & 0xFF) as u8;
		dotdot[21] = ((self.cluster >> 24) & 0xFF) as u8;
		dotdot[26] = (self.cluster & 0xFF) as u8;
		dotdot[27] = ((self.cluster >> 8) & 0xFF) as u8;
